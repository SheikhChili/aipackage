...
import optuna
import wandb
from aipackage.mlpackage.Entities import AccuracyEntity
...
class HyperParameterTuning:
    ...
    def set_actual_folder_name(self, folderName):
        self.actualFolderName = folderName
    ...
    # Fit the model
    # GRID SEARCH
    def model_grid_fit(self, modelEntity, fileName, folderPath):
        grid_search = GridSearchCV(estimator=modelEntity.alg, param_grid=modelEntity.grid, n_jobs=1,
        ...
        self.run_all_visualization(grid_search, self.hPEntity.cv, self.hPEntity.scoring, self.hPEntity.x_train,
                                   self.hPEntity.y_train, self.hPEntity.x_val, self.hPEntity.y_val, modelEntity.grid,
                                   fileName,
                                   self.hPEntity.model_type, folderPath)

        self.check_and_save_model(score, val_score, fileName, folderPath, grid_search)

        return AccuracyEntity(fileName, score, val_score, str(grid_search.best_params_))

    # BAYESIAN OPTIMIZATION
    def model_bayes_fit(self, modelEntity, fileName, folderPath):

        X_train = self.hPEntity.x_train
        Y_train = self.hPEntity.y_train
        model = modelEntity.alg
        grid = modelEntity.grid
        ...
        self.run_all_visualization(alg, self.hPEntity.cv, self.hPEntity.scoring, self.hPEntity.x_train,
                                   self.hPEntity.y_train, self.hPEntity.x_val, self.hPEntity.y_val, grid, fileName,
                                   self.hPEntity.model_type, folderPath)

        self.check_and_save_model(score, val_score, fileName, folderPath, alg)

        return AccuracyEntity(fileName, score, val_score, str(best))

    # GENETIC ALGORITHM
    def model_gene_fit(self, modelEntity, fileName, folderPath):
        ...
        if model_type == Variable.typeMultiClass:
            model = modelEntity.gene_alg
            grid = modelEntity.gene_grid
        else:
            model = modelEntity.alg
            grid = modelEntity.grid
        ...
        self.export_gene(tpot, fileName)
        ...
        self.check_and_save_model(score, val_score, fileName, folderPath, model)

        return AccuracyEntity(fileName, score, val_score, "example")

    # OPTUNA ALGORITHM
    def model_optuna_fit(self, modelEntity, fileName, folderPath):
        ...
        return AccuracyEntity(fileName, score, val_score, str(trial.params))

    # GENETIC ALGORITHM
    def model_sklearn_genetic_fit(self, modelEntity, fileName, folderPath):
        ga_search = GASearchCV(estimator=modelEntity.alg, param_grid=self.ga_search_param(modelEntity.grid), n_jobs=1,
        ...
        return AccuracyEntity(fileName, score, val_score, str(ga_search.best_params_))
    ...
    def export_gene(self, tpot, fileName):
        self.io.export_gene(tpot, fileName)
    ...
    def save_all_model(self, model_name, folderPath, model):
        self.io.save_all_model(model_name, folderPath, model)

    def save_all_visualizer(self, model_name, folderPath, model):
        self.io.save_all_visualizer(model_name, folderPath, model)
    ...
    def start_visualization(self, model, cv, scoring, X_train, Y_train, X_val, Y_val, grid, modelName, folderPath):
        print("startVisualization")
        matplotlib.use('Agg')
        self.start_learning_curve(model, cv, scoring, X_train, Y_train, modelName, folderPath)
        print("\n")
        # self.startValidationCurve(model, cv, scoring, X_val, Y_val, grid, modelName, folderPath)

    def start_learning_curve(self, model, cv, scoring, X, Y, modelName, folderPath):
        visualizer = LearningCurve(model, cv=cv, scoring=scoring, n_jobs=4)
        visualizer.fit(X, Y)  # Fit the data to the visualizer
        modelName = modelName + Variable.fileSeparator + Variable.learningCurve
        self.save_all_visualizer(modelName, folderPath, visualizer)
        # visualizer.savefig("mygraph.png")

    def start_validation_curve(self, model, cv, scoring, X, Y, grid, modelName, folderPath):
        for key, value in grid.items():
            visualizer = ValidationCurve(model, cv=cv, scoring=scoring, n_jobs=4, param_name=key, param_range=value)
            visualizer.fit(X, Y)  # Fit the data to the visualizer
            modelName = modelName + Variable.fileSeparator + key + Variable.fileSeparator + Variable.validationCurve
            self.save_all_visualizer(modelName, folderPath, visualizer)
        # visualizer.show()
